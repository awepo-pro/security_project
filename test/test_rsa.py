...
def read_file(msg_file):
	...
	with open(msg_file, 'rb') as target:
		msg_len, block_size, encrypted_msg = target.read().decode('utf-8').split('_')

	return int(msg_len), int(block_size), encrypted_msg

def rsa_decryption(msg, key=None):
	key_size, n, d = get_private_key(key)
	msg_len, block_size, encrypted_msg = msg.split('_')
	msg_len = int(msg_len)
